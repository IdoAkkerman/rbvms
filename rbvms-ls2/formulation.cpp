...
void NavStoLSForm::SetStrongBC (Array<int> strong_bdr)
{
   ...
   Array<int> ess_bdr_dist(fes[3]->GetMesh()->bdr_attributes.Max());
                                      ...
                                      const Vector &x0)
{
   ...
   xs2.SetSize(block_offsets[4]-block_offsets[3]);
...
Vector NavStoLSForm::GetEnergies(const Vector &x) const
{
   ...
   fes[3]->GetProlongationMatrix()->Mult(xs_true.GetBlock(3), xs.GetBlock(3));
