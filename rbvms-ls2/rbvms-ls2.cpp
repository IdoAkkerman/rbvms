...
int main(int argc, char *argv[])
{
   ...
   ParGridFunction x_dist(spaces[3]);
   ...
   if (restart && stat("restart/step.dat", &info) == 0)
   {
      ...
      if (nstate == 1)
      {
         ...
         *dx_phi[0] = *rdc.GetField("dphi");
         ...
         dx_phi[0]->GetTrueDofs(dxp.GetBlock(3));
   ...
   else
   {
      ...
      x_dist.ProjectCoefficient(sol_phi);
   ...
   Solver* pc_conv = nullptr;
   Solver* pc_rd   = nullptr;

   HypreILU* ilu_mom  = new HypreILU();
   HypreILU* ilu_cont = new HypreILU();
   HypreILU* ilu_conv = new HypreILU();
   HypreILU* ilu_rd   = new HypreILU();
   ...
   jac_prec.SetPreconditioner(2, pc_conv);
   jac_prec.SetPreconditioner(3, pc_rd);
   ...
   while (t < t_final)
   {
      ...
      while (t >= dt_vis*vi)
      {
         ...
         add (fac, xp0.GetBlock(3), (1.0-fac), xp.GetBlock(3), xpi.GetBlock(3));
         x_phi.Distribute(xpi.GetBlock(3));
      ...
      if (restart_interval > 0 && si%restart_interval == 0)
      {
         ...
         x_dist.Distribute(xp.GetBlock(3));
         if (nstate == 1)
         {
