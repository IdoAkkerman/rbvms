...
real_t IncNavStoIntegrator::GetRDTau(real_t &k, Vector &a, DenseMatrix &Gij)
{
   // Reaction part
   real_t  tau_i2 = k*k;
...
real_t IncNavStoIntegrator::GetRDKdc(real_t &res,
                                     Vector &dphidx,
                                     DenseMatrix &Gij)
{
   real_t h = 1.0/sqrt(Gij.Trace()/Gij.Width());
   return kdc_rd0*h + kdc_rd1*h*fabs(res)/fmax(dphidx.Norml2(), Heaviside::epsilon);
}
   ...
   Vector &energy)
{
   ...
   sh_dist.SetSize(dof_dist);
   shg_dist.SetSize(dof_dist, dim);
   ...
   for (int i = 0; i < ir.GetNPoints(); ++i)
   {
      ...
      el[3]->CalcPhysShape(Tr, sh_dist);
      real_t dist = sh_dist*(*elsol[3]);

      el[3]->CalcPhysDShape(Tr, shg_dist);
      shg_dist.MultTranspose(*elsol[3], grad_dist);
      real_t rho = GetRho(dist, grad_dist, Gij);
   ...
   real_t &elem_cfl)
{
   ...
   *elvec[3] = 0.0;
   ...
   for (int i = 0; i < ir.GetNPoints(); ++i)
   {
      ...
      real_t f = Se + lambda_rd*de*phi;
      real_t k = lambda_rd*de;
      a.Set(Se/fmax(grad_dist.Norml2(),Heaviside::epsilon), grad_dist);
...
std::cout<<"res_rd = "<<res_rd<<std::endl;
      ...
      shg_dist.Mult(a, test); // Add Convection stabilization term
      test.Add(k, sh_dist);     // Add Reaction stabilization term
      test *= tau_rd;            // Scale stabilization term with parameter
      test += sh_dist;        // Galerkin term

      // Weak residual
      elvec[3]->Add(w*res_ls, test);
   ...
   const Array2D<DenseMatrix *> &elmats)
{
   ...
   mat_dd.SetSize(dof_dist, dof_dist);
   ...
   Vector a(dim), test(dof_dist), trail(dof_dist);
   ...
   for (int i = 0; i < ir.GetNPoints(); ++i)
   {
      ...
      real_t rho = GetRho(dist, grad_dist, Gij);
      real_t drho = GetRhoGrad(dist, grad_dist, Gij);
      ...
      shg_dist.Mult(a, trail); // Add Convection term
      trail.Add(k, sh_dist);   // Add Reaction term

      // Compute test function
      test.Set(tau_rd, trail);   // Add stabilization term
      test += sh_dist;          // Add Galerkin term

      // Stablization term
      AddMult_a_VWt(w, test, trail, mat_dd);
                        ...
                        bool suction)
{
   ...
   *elvec[3] = 0.0;
   ...
   for (int i = 0; i < ir.GetNPoints(); i++)
   {
      ...
      el1[3]->CalcPhysShape(Tr, sh_dist);
      real_t dist = sh_dist*(*elsol[3]);

      el1[3]->CalcPhysDShape(Tr, shg_dist);
      shg_dist.MultTranspose(*elsol[3], grad_dist);
